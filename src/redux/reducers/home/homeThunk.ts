...
export const getHomeArticles = createAsyncThunk<PostType.ArticleStateProps[]>(
  ...
  async (payload, ThunkOptions) => {
    try {
      const {
        data: { data },
      }: RecentArticlesProps = await authorizedCustomAxios.get(`/posts/recent`);
...
>("home/getExtraArticle", async (payload, ThunkOptions) => {
  ...
  try {
    ...
    }: ExtraArticleProps = await authorizedCustomAxios.get(`/posts`, config); // 단건 조회 api 추가

    if (empty) {
      throw ThunkOptions.rejectWithValue("게시물이 더 이상 존재하지 않습니다.");
    }
...
>("home/postFollow", async (payload, ThunkOptions) => {
  try {
    const {
      data: { data: isSuccess },
      data,
    } = await authorizedCustomAxios.post(`/${payload.username}/follow`, null);
    if (
      data.code === "F006" ||
...
>("home/deleteLike", async (payload, ThunkOptions) => {
  ...
  try {
    const { data } = await authorizedCustomAxios.delete(`/posts/like`, config);
    return data;
  } catch (error) {
    error === FAIL_TO_REISSUE_MESSAGE &&
      ThunkOptions.dispatch(authAction.logout());
