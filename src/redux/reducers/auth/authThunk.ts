      ...
      if (!window.navigator.onLine) {
        ...
        const checkUsername = async () => {
          try {
            ...
            const { data } = await customAxios.get(`/accounts/check`, config);
            return data;
          } catch (error) {
...
export const getUserInfo = createAsyncThunk<AuthType.UserInfo>(
  ...
  async (payload, ThunkOptions) => {
    try {
      const response = await authorizedCustomAxios.get("/accounts/profile");
...
>("auth/checkResetPassword", async (payload, ThunkOptions) => {
  try {
    ...
    const { data } = await customAxios.get(`/accounts/password/reset`, config);
    console.log(data);
  } catch (error) {
    throw ThunkOptions.rejectWithValue(error);
  }
